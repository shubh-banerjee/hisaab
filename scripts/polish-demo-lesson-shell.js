...
const demoMarkup = `    <section class="demo-intro demo-lesson" id="demo-intro" hidden aria-live="polite">
      <div class="demo-lesson-frame" aria-label="Hisaab demo lesson">
        ...
        <section class="demo-step" id="demo-step-questions" hidden>
          <div class="demo-step-content demo-step-content-questions">
            <div class="demo-eyebrow">Choose a question</div>
            <h2>Pick what the shop owner wants to understand</h2>
            <p>These are real questions a small business owner might ask.</p>
            <div class="demo-question-list">
              <button class="demo-question" type="button" data-demo-question="trend"><strong>Are orders going up or down?</strong><span>Check whether the shop is growing, slowing, or having a normal month.</span></button>
        ...
        <section class="demo-step demo-result-step" id="demo-step-result" hidden>
          <div class="demo-step-content demo-step-content-result">
...
body.view-demo-intro .stage,body.view-demo-found-data .stage,body.view-demo-questions .stage,body.view-demo-result .stage{min-height:100svh;justify-content:center;align-items:center;padding:48px 24px;}
...
.demo-frame-progress{height:72px;flex:0 0 72px;display:grid;grid-template-columns:44px 1fr auto;align-items:center;gap:18px;padding:0 42px 0 28px;border-bottom:1px solid rgba(13,24,51,.10);color:var(--ink-mute);font-size:11px;font-weight:780;letter-spacing:.08em;text-transform:uppercase;}
.demo-close{width:40px;height:40px;display:grid;place-items:center;border:0;border-radius:50%;background:transparent;color:var(--ink-mute);font-size:32px;line-height:1;cursor:pointer;transition:color .16s ease,background .16s ease;}
...
.demo-step-content{flex:1;min-height:0;display:flex;flex-direction:column;justify-content:center;padding:40px 70px 32px;overflow:hidden;}.demo-step-content-narrow{max-width:640px;align-self:center;width:100%;}.demo-step-content-questions{padding-top:34px;padding-bottom:26px;}.demo-step-content-result{padding-top:34px;padding-bottom:24px;}
...
.demo-lesson .demo-step h2{max-width:760px;margin:0;color:var(--ink);font-size:clamp(32px,3.4vw,42px);font-weight:760;letter-spacing:-.045em;line-height:1.08;}
.demo-lesson .demo-step p{max-width:650px;margin:14px 0 0;color:var(--ink-soft);font-size:16px;line-height:1.5;}
.demo-simple-points{display:flex;flex-wrap:wrap;gap:8px;margin-top:24px;}.demo-simple-points span{padding:7px 11px;border:1px solid rgba(13,24,51,.14);border-radius:999px;color:var(--ink-soft);background:transparent;font-size:12.5px;line-height:1.2;}
.demo-found-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin:24px 0 0;padding:0;list-style:none;}.demo-found-list div,.demo-result-section{border:1px solid rgba(13,24,51,.13);border-radius:16px;background:transparent;}.demo-found-list div{min-height:82px;padding:15px 16px;}.demo-found-list strong{display:block;color:var(--ink);font-size:14px;line-height:1.3;}.demo-found-list span{display:block;margin-top:6px;color:var(--ink-soft);font-size:12.5px;line-height:1.38;}
.demo-example-note{color:var(--ink-mute)!important;font-size:12.5px!important;font-style:normal!important;}
.demo-question-list{display:grid;grid-template-columns:1fr;gap:9px;margin-top:22px;}.demo-question{min-height:62px;padding:13px 16px;border:1.5px solid rgba(13,24,51,.15);border-radius:15px;background:transparent;color:var(--ink);text-align:left;box-shadow:none;transition:border-color .16s ease,background .16s ease,transform .16s ease;}.demo-question:hover,.demo-question:focus-visible{border-color:var(--accent);background:rgba(53,109,255,.05);transform:translateY(-1px);outline:none;}.demo-question strong,.demo-question span{display:block;}.demo-question strong{color:var(--ink);font-size:14px;font-weight:720;line-height:1.3;}.demo-question span{margin-top:4px;color:var(--ink-soft);font-size:12px;line-height:1.34;}
.demo-result-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:20px;}.demo-result-section{margin:0;padding:14px 15px;}.demo-result-section h3{margin:0 0 5px;color:var(--ink);font-size:12.5px;font-weight:780;}.demo-result-section p{margin:0;color:var(--ink-soft);font-size:12.5px;line-height:1.42;}.demo-answer-block{grid-column:1/-1;}
...
@media(max-height:790px) and (min-width:761px){body.view-demo-intro .stage,body.view-demo-found-data .stage,body.view-demo-questions .stage,body.view-demo-result .stage{padding:30px 24px;}.demo-lesson-frame{height:590px;}.demo-frame-progress{height:66px;flex-basis:66px;}.demo-step-footer{height:76px;flex-basis:76px;}.demo-step-content{padding-top:28px;padding-bottom:22px;}.demo-lesson .demo-step h2{font-size:34px;}.demo-question{min-height:58px;}.demo-found-list div{min-height:76px;}}
@media(max-width:760px){body.view-demo-intro .stage,body.view-demo-found-data .stage,body.view-demo-questions .stage,body.view-demo-result .stage{padding:24px 14px;}.demo-lesson-frame{width:calc(100vw - 28px);height:min(680px,calc(100svh - 48px));min-height:600px;border-radius:22px;}.demo-frame-progress{height:66px;flex-basis:66px;grid-template-columns:40px 1fr auto;padding:0 18px 0 12px;gap:10px;}.demo-progress-dots i{width:18px;}.demo-step-content{padding:30px 22px 24px;}.demo-lesson .demo-step h2{font-size:30px;}.demo-lesson .demo-step p{font-size:15px;}.demo-found-list,.demo-result-grid{grid-template-columns:1fr;}.demo-step-footer,.demo-result-actions-row{height:auto;min-height:86px;flex-basis:auto;flex-direction:column-reverse;align-items:stretch;padding:14px 18px;}.demo-nav-primary,.demo-nav-secondary,.demo-lesson .cta-primary,.demo-lesson .cta-secondary{width:100%;}}
/* End Hisaab demo lesson v4 */`;
