...
function hbwQuestions(summary) {
  ...
  if (usable('pricing')) q.push('What happens if I change my prices?');
  if (usable('delivery_fee')) q.push('Should I raise my delivery fee?');
  if (usable('promotions')) q.push('Are my discounts actually working?');
...
const CLIENT_HELPER = String.raw`
  ...
  function renderBusinessResult(data, elapsed = 0) {
    ...
    const factsHtml = facts.length ? '<div class="br-facts">' + facts.map(fact => '<span>' + escapeHtml(fact) + '</span>').join('') + '</div>' : '';
    const limitHtml = bundle.limitation ? '<div class="br-limitation">' + escapeHtml(bundle.limitation) + '</div>' : '';
